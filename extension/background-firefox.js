 ...
 * Firefox Manifest V2 compatible (no ES6 modules)
...
class FormFiller {
  static isRestrictedUrl(url) {
    ...
    return restrictedPrefixes.some(prefix => url && url.startsWith(prefix));
  }
}
...
async function handleFillCardForm(data) {
  try {
    ...
    if (FormFiller.isRestrictedUrl(tab.url)) {
      ...
      try {
        // Firefox MV2 uses tabs.executeScript instead of scripting.executeScript
        await browserAPI.tabs.executeScript(tab.id, {
          code: 'alert(\'⚠️ Cannot use card fill on this page\\n\\nPlease use on a payment page (e.g., cursor.com checkout)\');'
        });
      } catch (notifyError) {
    ...
    await browserAPI.tabs.executeScript(tab.id, {
      code: `(${selectCardPaymentMethod.toString()})()`
    });
    ...
    await browserAPI.tabs.executeScript(tab.id, {
      code: `(${executeCardFillProcess.toString()})(${JSON.stringify(data)})`
    });
